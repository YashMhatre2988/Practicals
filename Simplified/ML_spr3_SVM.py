...
X_proc = PCA(n_components=min(5, X_sel.shape[1])).fit_transform(
...
# Linear SVM
linear_svm = SVC(kernel='linear', random_state=42).fit(X_train, y_train)
print("\nLinear SVM Accuracy:", accuracy_score(y_test, linear_svm.predict(X_test)))
print(classification_report(y_test, linear_svm.predict(X_test)))

# Non-Linear SVM (RBF kernel)
rbf_svm = SVC(kernel='rbf', gamma='scale', random_state=42).fit(X_train, y_train)
...
# Save processed data
out = pd.DataFrame(X_proc, columns=[f'PC{i+1}' for i in range(X_proc.shape[1])])
